...
namespace ChampionsOfForest.Network
{
	public class CommandReader
	{
		...
		public static void OnCommand(byte[] bytes)
		{
			...
			{
				var command = (INetworkCommand)new BinaryFormatter().Deserialize(stream);
				command.Execute();
						...
						case 5:
							{
								...
								{
									ItemStat stat =
										new ItemStat(ItemDataBase.Stats[r.ReadInt32()], itemLvl, r.ReadInt32())
										{
											Amount = r.ReadSingle()
										};
									item.Stats.Add(stat);
								...
								if (!GameSetup.IsMpClient)
								{
									...
									if (EnemyManager.hostDictionary.ContainsKey(packed))
									{
										...
										using (MemoryStream answerStream = new MemoryStream())
										{
											...
											Network.NetworkManager.SendLine(answerStream.ToArray(),
												Network.NetworkManager.Target.Clients);
											answerStream.Close();
										}
									}
									else
						...
						case 7:
							{
								...
								if (GameSetup.IsMpClient)
								{
									...
									if (EnemyManager.allboltEntities.ContainsKey(packed))
									{
										...
										{
											new ClinetEnemyProgression(entity, name, level, health, maxhealth,
												bounty, armor, armorReduction, steadfast, affixes);
										}
									}
								}
								...
								if (ModdedPlayer.Stats.rootImmunity == 0 && ModdedPlayer.Stats.stunImmunity == 0)
								{
									...
									if ((LocalPlayer.Transform.position - pos).sqrMagnitude < 1250)
									{
										...
										using (MemoryStream answerStream = new MemoryStream())
										{
											...
											NetworkManager.SendLine(answerStream.ToArray(),
												NetworkManager.Target.Everyone);
											answerStream.Close();
										}
									}
								}
								...
								if (GameSetup.IsMpServer)
								{
									...
									if (PickUpManager.PickUps.ContainsKey(itemID))
									{
										...
										if (PickUpManager.PickUps[itemID].amount > 0)
										{
											...
											{
												givenAmount = Mathf.Min(PickUpManager.PickUps[itemID].amount,
													itemAmount);
											}

											NetworkManager.SendItemToPlayer(PickUpManager.PickUps[itemID].item,
												playerID, givenAmount);
						...
						case 26:
							{
								...
								{
									//creating the item.
									Item item = new Item(ItemDataBase.ItemBases[r.ReadInt32()], r.ReadInt32(), 0,
										false) {level = r.ReadInt32()};
						...
						case 28:
							{
								...
								if (!ModReferences.PlayerHands.ContainsKey(id) ||
								    ModReferences.PlayerHands[id] == null)
								{
								...
								switch (ptype)
								{
									case MarkObject.PingType.Enemy:
										...
										if (EnemyManager.allboltEntities.ContainsKey(EnemyID))
										{
											...
											{
												MainMenu.Instance.localPlayerPing =
													new MarkEnemy(tr, name, isElite);
											}
											else
											{
												if (MainMenu.Instance.otherPlayerPings.ContainsKey(PlayerID))
												{
													MainMenu.Instance.otherPlayerPings[PlayerID] =
														new MarkEnemy(tr, name, isElite);
												}
												else
												{
													MainMenu.Instance.otherPlayerPings.Add(PlayerID,
														new MarkEnemy(tr, name, isElite));
												}
											}
										}
									...
									case MarkObject.PingType.Location:
										...
										{
											MainMenu.Instance.localPlayerPing =
												new MarkPostion(new Vector3(x, y, z));
										}
										else
										{
											if (MainMenu.Instance.otherPlayerPings.ContainsKey(PlayerID))
									...
									case MarkObject.PingType.Item:
										...
										if (PickUpManager.PickUps.ContainsKey(PickupID))
										{
											...
											if (PlayerID == ModReferences.ThisPlayerID)
											{
												MainMenu.Instance.localPlayerPing = new MarkPickup(pu.transform,
													pu.item.name, pu.item.Rarity);
											}
											else
											{
												if (MainMenu.Instance.otherPlayerPings.ContainsKey(PlayerID))
