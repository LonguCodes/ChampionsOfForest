...
namespace ChampionsOfForest.ExpSources
{
	public class FishEX : Fish
	{
		...
		public void SharkKilled()
		{
			...
			{
				Network.NetworkManager.SendItemDrop(ItemDataBase.GetRandomItem(270),
					LocalPlayer.Transform.position + Vector3.up * 6f);
				Network.NetworkManager.SendItemDrop(ItemDataBase.GetRandomItem(270),
					LocalPlayer.Transform.position + Vector3.up * 6f);
				Network.NetworkManager.SendItemDrop(ItemDataBase.GetRandomItem(370),
					LocalPlayer.Transform.position + Vector3.up * 6f);
			}
		}
	}
