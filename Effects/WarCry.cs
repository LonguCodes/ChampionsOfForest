...
namespace ChampionsOfForest.Effects
{
	public class WarCry
	{
		...
		public static void CastFromData(WarCryData data)
		{
			Cast(data.Pos, data.Radius, data.Radius, data.Dmg, data.GiveDmg, data.GiveAr, data.Ar);
		}

		public static void Cast(Vector3 pos, float radius, float speed, float damage, bool GiveDamage,
			bool GiveArmor = false, int ArmorAmount = 1)
		{
		...
		public static void GiveEffect(float speed, float damage, bool giveEffect2, bool giveEffect3,
			int ArmorAmount = 1)
		{
		...
		public static void SpawnEffect(Vector3 pos, float radius)
		{
			...
			if (particleMaterial == null)
			{
				...
				{
					mainTexture = Res.ResourceLoader.GetTexture(111), mainTextureScale = new Vector2(30, 1)
				};
