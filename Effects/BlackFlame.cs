...
namespace ChampionsOfForest.Effects
{
	public class BlackFlame : MonoBehaviour
	{
		...
		public static float FireDamageBonus =>
			(30 + ModdedPlayer.Stats.spellFlatDmg / ModdedPlayer.Stats.spell_blackFlameDamageScaling) *
			ModdedPlayer.Stats.TotalMagicDamageMultiplier * DmgAmp / 3;
		...
		public static void CreateFromData(BlackFlameData data)
		{
			ToggleOtherPlayer(data.PlayerName, data.IsOn);
		}

		public static GameObject Create()
		{
			AnimationCurve sizecurve = new AnimationCurve(new Keyframe[]
			{
				new Keyframe(0, 0, 5.129462f, 5.129462f), new Keyframe(0.2449522f, 1, 0, 0),
				new Keyframe(1, 0, -1.242162f, -1.242162f),
			});
			if (mat1 == null)
			{
			...
			if (BoltNetwork.isRunning)
			{
				using (System.IO.MemoryStream answerStream = new System.IO.MemoryStream())
				{
					...
					ChampionsOfForest.Network.NetworkManager.SendLine(answerStream.ToArray(),
						ChampionsOfForest.Network.NetworkManager.Target.Others);
					answerStream.Close();
				}
			}
		}
