...
namespace ChampionsOfForest
{
	public class DarkBeam : MonoBehaviour
	{
		...
		public static void CastFromData(DarkBeamData data)
		{
			Create(data.Pos, data.FromEnemy, data.Damage, data.Healing, data.Slow, data.Boost, data.Duration,
				data.Radius);
		}
		...
		/// </summary>
		public static void Create(Vector3 position, bool fromEnemy, float Damage, float Healing, float Slow,
			float Boost, float duration = 10f, float Radius = 3.5f)
		{
			try
			{
				...
				foreach (RaycastHit hit in hits)
				{
					if (hit.transform.CompareTag("enemyCollide"))
					{
						...
						if (ep != null)
						{
							if (fromEnemy)
							{
								ep.HealthScript.Health = (int)Mathf.Clamp(ep.HealthScript.Health + Healing / 2, 0,
									ep.maxHealth);
								ep.Slow(6, Boost, 25);
							}
							else
		...
		private void Update()
		{
			...
			if (EffectReady)
			{
				...
				if ((LocalPlayer.Transform.position - transform.position).sqrMagnitude < Radius * Radius)
				{
					if (fromEnemy)
					{
						LocalPlayer.Stats.HealthChange(-Damage * Time.deltaTime *
